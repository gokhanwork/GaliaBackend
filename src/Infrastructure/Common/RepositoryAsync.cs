...
using Galia.Application.Constants;
...
namespace Galia.Infrastructure.Persistence.Repositories;
...
public class RepositoryAsync : IRepositoryAsync
{
    ...
    public async Task<List<T>> GetListForSpecAsync<T>(Expression<Func<T, bool>> expression, bool noTracking = false, BaseSpecification<T> specification = null, CancellationToken cancellationToken = default)
        where T : BaseEntity
    {
        IQueryable<T> query = _dbContext.Set<T>();
    ...
    public async Task<IReadOnlyList<TDto>> QueryAsync<T, TDto>(string sql, object param = null, IDbTransaction transaction = null, CancellationToken cancellationToken = default)
        where T : BaseEntity
        where TDto : IDto
    {
        if (typeof(IMustHaveTenant).IsAssignableFrom(typeof(T)))
        ...
        return (await _dbContext.Connection.QueryAsync<TDto>(sql, param, transaction)).AsList();
    }
    ...
    public async Task<PaginatedResult<TDto>> GetSearchResultsAsync<T, TDto>(int pageNumber, BaseSpecification<T> specification = null, int pageSize = int.MaxValue, string[] orderBy = null, Filters<T> filters = null, Search advancedSearch = null, string keyword = null, CancellationToken cancellationToken = default)
        where T : BaseEntity
        where TDto : IDto
    {
            IQueryable<T> query = _dbContext.Set<T>();
