...
public class ProductService : IProductService
{
    ...
    {
        bool productExists = await _repository.ExistsAsync<Product>(a => a.Code == request.Code);
    ...
    public async Task<Result<ProductDetailsDto>> GetProductDetailsAsync(Guid id)
    {
        ...
        spec.Includes.Add(a => a.Category);
    ...
    public async Task<PaginatedResult<ProductDto>> SearchAsync(ProductListFilter filter)
    {
        ...
        filters.Add(filter.SubCategoryId.HasValue, x => x.SubCategoryId.Equals(filter.SubCategoryId.Value));
        ...
        spec.Includes.Add(a => a.Brand);
        spec.Includes.Add(a => a.Category);
        spec.Includes.Add(a => a.SubCategory);
        return await _repository.GetSearchResultsAsync<Product, ProductDto>(filter.PageNumber, spec, filter.PageSize, filter.OrderBy, filters, filter.AdvancedSearch, filter.Keyword);
    }

    public async Task<Result<List<ProductPosDto>>> GetProductPosList()
    {
        var products = await _repository.GetListAsync<Product>(null, true);
        var mappedProducts = products.Adapt<List<ProductPosDto>>();
        return await Result<List<ProductPosDto>>.SuccessAsync(mappedProducts);
