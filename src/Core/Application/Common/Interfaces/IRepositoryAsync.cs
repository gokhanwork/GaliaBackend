...
public interface IRepositoryAsync : ITransientService
{
    ...
    where T : BaseEntity;
    Task<List<T>> GetListForSpecAsync<T>(Expression<Func<T, bool>> expression, bool noTracking = false, BaseSpecification<T> specification = null, CancellationToken cancellationToken = default)
        where T : BaseEntity;
    ...
    where T : BaseEntity;
    Task<IReadOnlyList<TDto>> QueryAsync<T, TDto>(string sql, object param = null, IDbTransaction transaction = null, CancellationToken cancellationToken = default)
    where T : BaseEntity
    where TDto : IDto;
    ...
    Task<PaginatedResult<TDto>> GetSearchResultsAsync<T, TDto>(int pageNumber, BaseSpecification<T> specification = null, int pageSize = int.MaxValue, string[] orderBy = null, Filters<T> filters = null, Search advancedSearch = null, string keyword = null, CancellationToken cancellationToken = default)
        where T : BaseEntity
        where TDto : IDto;
